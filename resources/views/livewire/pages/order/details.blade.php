<div>
    <flux:modal name="add-product-to-order" class="md:w-300">
        <div class="space-y-6">
            ...
            <flux:input wire:model.live.debounce.500ms="search" label="{{ __('Product') }}" placeholder="{{ __('Search Product') }}" />
                ...
                @foreach($products as $product)
                    <div class="flex justify-between items-center w-full">
                        <div class="flex justify-start items-center space-x-4">
                            ...
                            <p class="text-base dark:text-white font-semibold leading-4 text-gray-800">{{ $product['name'] }}</p>
                        </div>
                        <flux:button wire:click="addProductToOrder({{ $product['id'] }})" variant="primary" size="xs" icon="plus">{{ __('Add') }}</flux:button>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        <div class="flex justify-start item-start space-y-2 flex-col">
            <h1 class="text-3xl dark:text-white lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">{{ __('Order') }}
                #{{ $order['id'] }}</h1>
        ...
        <div class="mt-5 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
            <div class="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
                <div class="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                    <p class="text-lg md:text-xl dark:text-white font-semibold leading-6 xl:leading-5 text-gray-800">{{ __('Customer’s Cart') }}</p>
                    ...
                    <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
                            ...
                            <div class="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                                ...
                                <div class="flex justify-between space-x-8 items-start w-full">
                                    <p class="text-base dark:text-white xl:text-lg leading-6">{{ $order['currency_symbol'] }}{{ number_format($item['price'], 2) }}
...
{{--                                        <span class="text-red-300 line-through"> $45.00</span>--}}
                                    ...
                                    <p class="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">{{ $order['currency_symbol'] }}{{ $item['subtotal'] }}</p>
                                </div>
                            </div>
                    </div>
                    @endforeach
                ...
                <div class="flex justify-center flex-col md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
                    <div class="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                        <h3 class="text-xl dark:text-white font-semibold leading-5 text-gray-800">{{ __('Summary') }}</h3>
                        <div class="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                            ...
                            <div class="flex justify-between items-center w-full">
                                ...
                                <p class="text-base dark:text-gray-300 leading-4 text-gray-600">{{ number_format($order['discount_total'], 2) }}</p>
                            </div>
                            <div class="flex justify-between items-center w-full">
                                <p class="text-base dark:text-white leading-4 text-gray-800">{{ __('Shipping') }}</p>
                                <p class="text-base dark:text-gray-300 leading-4 text-gray-600">{{ number_format($order['shipping_total'], 2) }}</p>
                            </div>
                        </div>
                        <div class="flex justify-between items-center w-full">
                            <p class="text-base dark:text-white font-semibold leading-4 text-gray-800">{{ __('Total') }}</p>
                            <p class="text-base dark:text-gray-300 font-semibold leading-4 text-gray-600">{{ number_format($totalAmount, 2) }}</p>
                        </div>
                    </div>
                    <div class="flex flex-col justify-center px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                        <h3 class="text-xl dark:text-white font-semibold leading-5 text-gray-800">{{ __('Shipping') }}</h3>
                ...
                <div class="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
                    <div class="flex flex-col justify-start items-start flex-shrink-0">
                        <div class="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                            ...
                            <div class="flex justify-start items-start flex-col space-y-2">
                                <p class="text-base dark:text-white font-semibold leading-4 text-left text-gray-800">{{ $order['billing']['first_name'] . ' ' . $order['billing']['last_name'] }}</p>
{{--                                <p class="text-sm dark:text-gray-300 leading-5 text-gray-600">10 Previous Orders</p>--}}
                    ...
                    <div class="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
                            ...
                            <div class="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
                                ...
                                @if(!empty($order['shipping']))
                                    <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
                                        <p class="text-base dark:text-white font-semibold leading-4 text-center md:text-left text-gray-800">{{ __('Shipping Address') }}</p>
                                        @if(!empty($order['shipping']['first_name']) || !empty($order['shipping']['last_name']))
                                            <p class="...">{{ $order['shipping']['first_name'] . ' ' . $order['shipping']['last_name'] }}</p>
                                        @endif
                                ...
                                @if(!empty($order['billing']))
                                    <div class="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4">
                                        <p class="text-base dark:text-white font-semibold leading-4 text-center md:text-left text-gray-800">{{ __('Billing Address') }}</p>
                                        @if(!empty($order['billing']['first_name']) || !empty($order['billing']['last_name']))
                                            <p class="...">{{ $order['billing']['first_name'] . ' ' . $order['billing']['last_name'] }}</p>
                                        @endif
                        ...
                        <div class="flex w-full justify-center items-center md:justify-start md:items-start">
                            <button class="mt-6 md:mt-0 dark:border-white dark:hover:bg-gray-900 dark:bg-transparent dark:text-white py-5 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-gray-800 font-medium w-96 2xl:w-full text-base font-medium leading-4 text-gray-800">{{ __('Edit Details') }}</button>
                        </div>
                    </div>
                </div>
            </div>
