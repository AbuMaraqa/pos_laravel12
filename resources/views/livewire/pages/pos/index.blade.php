            ...
            for (const item of filtered) {
                ...
                div.onclick = async function () {
                    if (item.type === 'variable') {
                        // 1) جرّب تقرأ المتغيرات من store "variations" عبر index product_id
                        const txVar = db.transaction("variations", "readonly");
                        const storeVar = txVar.objectStore("variations");
                        let variations = [];

                        if (storeVar.indexNames.contains("product_id")) {
                            const idx = storeVar.index("product_id");
                            const range = IDBKeyRange.only(item.id);
                            await new Promise((resolve) => {
                                const req = idx.getAll(range);
                                req.onsuccess = () => { variations = req.result || []; resolve(); };
                                req.onerror = () => resolve();
                            });
                        }

                        // 2) لو لسا فاضية، نستدعي جلب كل المتغيرات (مرّة) ثم نعيد المحاولة بسرعة
                        if (!variations.length) {
                            // اطلب تعبئة المتغيرات للسيرفر (اسم الحدث المصحّح)
                            Livewire.dispatch('fetch-all-variations');
                            // انتظر قليلًا ثم اقرأ ثانية
                            await new Promise(r => setTimeout(r, 600));

                            const txVar2 = db.transaction("variations", "readonly");
                            const storeVar2 = txVar2.objectStore("variations");
                            if (storeVar2.indexNames.contains("product_id")) {
                                const idx2 = storeVar2.index("product_id");
                                const range2 = IDBKeyRange.only(item.id);
                                await new Promise((resolve) => {
                                    const req2 = idx2.getAll(range2);
                                    req2.onsuccess = () => { variations = req2.result || []; resolve(); };
                                    req2.onerror = () => resolve();
                                });
                            }
                        }

                        // 3) لو لقينا متغيرات في IndexedDB، افتح المودال
                        if (variations.length) {
                            showVariationsModal(variations);
                            return;
                        }

                        // 4) مسار احتياطي: لو المنتج يحتوي IDs للمتغيرات ومخزّنة كـ "products"
                        ...
                        // 5) لو ولا مسار اشتغل
                        alert("جارٍ تجهيز المتغيرات… أعد المحاولة بعد لحظات");
